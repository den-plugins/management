module ResourceManagementsHelper
  ...
  def resource_countby(skill, week=nil, acctg=nil)
    if @resources_no_limit
      ...
      if week.nil?
        ...
        resources.select {|r| r.field_skill == skill and !r.is_resigned and !r.allocations(week, @projects).zero?}.count
      end
    end
  end
  ...
  def get_total_allocations_per_skill(skill, week, acctg)
    ...
    if @resources_no_limit
      resources = @resources_no_limit - @resources
      resources.each {|r| total_alloc_per_skill += r.allocations(week, @projects) if r.field_skill == skill and !r.is_resigned}
    end
